...
All files within the same folder point to the same module doc. The comment token is determined by file extension using `comment_map` in `config/dok-fu.config.json`.
...
- **Pointer broken / file renamed?** → Run `dokfu doctor`, then repair manually following the pointer format rules above
