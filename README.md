...
One doc module per **source directory**. A module documents a folder, not a single file. Each H2 section documents exactly one source file within that folder. Root-level files are documented in `docs/root.md` with `code: .` in the frontmatter.
...
- `code`: repo-relative path to the **source folder**; must resolve to an existing directory
...
Placed after shebang/package declaration and before imports. The comment token is determined by file extension using `comment_map` in `config/dok-fu.config.json`. All files within the same folder point to the same module doc.
...
Never hand-edited. Regenerated by `python scripts/dokfu.py index`.
